...
export const editCourse = async (req, res) => {
    try {
        ...
        const newThumbnail = req.file; // This will be undefined if no file is uploaded
        ...
        // THE FIX: Check if newThumbnail exists before accessing .path
        if (newThumbnail) {
            ...
            const newThumbnailUpload = await uploadMedia(newThumbnail.path);
        ...
        console.error("SERVER CRASH:", error); // This helps you see the error in the terminal
...
export const togglePublishCourse = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message:
        "Course is " + (course.isPublished ? "published" : "unpublished"),
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
